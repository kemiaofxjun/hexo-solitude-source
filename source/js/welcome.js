...
// 进行 fetch 请求
fetch('https://api.nsmao.net/api/ipip/query?key=SPVYL8BGTAD81rxMmLv7jMHtdz') //申请key:https://api.nsmao.net
    .then(response => {
        if (!response.ok) {
...
function showWelcome() {
    ...
    let dist = getDistance(111.749181, 40.842585, ipLocation.data.lng, ipLocation.data.lat); // 修改自己的经度（121.413921）纬度（31.089290）
...
// 那年今日
 ...
 async function fetchHistoryData() {
     ...
     const historyDataUrl = `https://api.nsmao.net/api/history/query?key=SPVYL8BGTAD81rxMmLv7jMHtdz`; //申请：https://api.nsmao.net
